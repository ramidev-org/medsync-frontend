...
export default function OrdonnancesTab({
  ...
  // The database now rejects any content change (or moving off "signed") to
  // an already-signed prescription (see enforce_prescription_immutability in
  // 2026_08_28_workflow_state_machine_fixes.sql). Until now the UI only knew
  // about the whole-consultation `readOnly` state, so a doctor could keep
  // editing/un-validating a prescription that had already been signed while
  // the consultation was still open, and only find out it failed after the
  // save was rejected. Lock it here instead, the same way `readOnly` is used.
  const isSelectedRxSigned = selectedRx?.status === "signed";
  const rxLocked = readOnly || isSelectedRxSigned;
  ...
  const persistPrescription = async (nextPrescription: Prescription): Promise<Prescription | null> => {
    if (!requesterId || !consultationId || !patientId) return null;
    setSaving(true);
    setError(null);
    const nextStatus =
      nextPrescription.drugs.length > 0 && nextPrescription.drugs.every((drug) => drug.validated)
        ? "signed"
        : "draft";
    try {
      ...
      // The RPC computes status the same way we just did, so reflect it
      // locally right away instead of waiting on a refetch - this is what
      // rxLocked reads to freeze the UI once a prescription is signed.
      return { ...nextPrescription, status: nextStatus };
    } catch (err) {
  ...
  const patchSelectedPrescription = async (
    ...
    const previous = selectedRx;
    const nextPrescription = patcher(selectedRx);
    // Optimistic update for instant feedback (e.g. the validated checkbox
    // flipping immediately) - reverted below if the save is rejected, so a
    // failed write (e.g. the DB refusing to un-sign a signed prescription)
    // doesn't leave the UI showing a state the server never accepted.
    setPrescriptions((prev) =>
      prev.map((row) => (row.id === nextPrescription.id ? nextPrescription : row)),
    );
    const saved = await persistPrescription(nextPrescription);
    setPrescriptions((prev) =>
      prev.map((row) => (row.id === nextPrescription.id ? (saved ?? previous) : row)),
    );
  };
  ...
  const addDrugFromCatalog = async (drug: DrugSuggestion) => {
    ...
    if (!active) return;
    if (active.status === "signed") {
      setError("Cette ordonnance est déjà signée et ne peut plus être modifiée.");
      return;
    }
    ...
    const saved = await persistPrescription(nextPrescription);
    setPrescriptions((prev) => prev.map((row) => (row.id === active.id ? (saved ?? previous) : row)));
  };
  ...
  const saveExpandedDrug = async () => {
    ...
    if (!selectedRx) return;
    const previous = selectedRx;
    const saved = await persistPrescription(selectedRx);
    setPrescriptions((prev) => prev.map((row) => (row.id === selectedRx.id ? (saved ?? previous) : row)));
    setExpandedDrugId(null);
  };

  const saveCurrentPrescription = async () => {
    if (rxLocked || !selectedRx) return;
    ...
    const saved = await persistPrescription(nextPrescription);
    setPrescriptions((current) => current.map((row) => row.id === nextPrescription.id ? (saved ?? previous) : row));
  };
            ...
            {catalogLoading ? (
              ...
              return (
                <TouchableOpacity
                  ...
                  disabled={rxLocked || (limitReached && !selectedDrug)}
                >
          ...
          <View style={styles.rightHeaderRow}>
            ...
            <View style={styles.headerActions}>
              ...
              {overflowMenuOpen && (
                <View style={styles.overflowMenu}>
                  <TouchableOpacity style={styles.overflowMenuItem} onPress={saveCurrentPrescription} disabled={saving || rxLocked || !selectedRx}>
                  ...
                  <TouchableOpacity style={styles.overflowMenuItem} onPress={() => { setOverflowMenuOpen(false); resetCurrentPrescription(); }} disabled={rxLocked || !selectedRx}>
            ...
            {isSelectedRxSigned ? (
              <View style={styles.signedBadge}>
                <Ionicons name="lock-closed" size={13} color={theme.colors.success} />
                <Text style={styles.signedBadgeText}>Ordonnance signée — lecture seule</Text>
              </View>
            ) : null}
              ...
              {displayedRx?.drugs?.length ? (
                <View style={styles.rxDrugPreview}>
                  {displayedRx.drugs.map((drug) => {
                    ...
                    const editable = !templatePreview && selectedRx?.id === displayedRx.id && !rxLocked;
                    return (
                ...
                return (
                  <View key={d.id} style={styles.drugCard}>
                    <TouchableOpacity style={styles.drugRow} onPress={() => setExpandedDrugId((cur) => (cur === d.id ? null : d.id))}>
                      ...
                      <TouchableOpacity onPress={() => toggleValidated(d.id)} style={styles.rowIconBtn} disabled={rxLocked}>
                        <Ionicons
                      ...
                      <TouchableOpacity onPress={() => deleteDrug(d.id)} style={styles.rowIconBtn} disabled={rxLocked}>
                    ...
                    {expanded && (
                      <View style={styles.drugEditor}>
                        ...
                        <TouchableOpacity style={[styles.saveBtn, { backgroundColor: theme.colors.primary }, rxLocked && { opacity: 0.7 }]} onPress={saveExpandedDrug} disabled={rxLocked}>
                          <Ionicons name="save-outline" size={16} color={theme.colors.textOnPrimary} />
                          <Text style={styles.saveBtnText}>{rxLocked ? "LECTURE SEULE" : "ENREGISTRER"}</Text>
                        </TouchableOpacity>
                      </View>
                    )}
                  </View>
...
const createStyles = (theme: any) =>
  StyleSheet.create({
    ...
    signedBadge: {
      ...
      paddingVertical: 6,
      borderRadius: 999,
      backgroundColor: theme.colors.soft?.success ?? "#ECFDF5",
    },
    signedBadgeText: {
