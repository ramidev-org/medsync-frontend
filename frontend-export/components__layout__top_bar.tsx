...
import { QuickSearch } from "@/components/layout/quick_search";
...
export const TopBar: React.FC<TopBarProps> = ({ theme }) => {
  ...
  const chatsOpenRef = useRef(chatsOpen);
  chatsOpenRef.current = chatsOpen;

  // Live "new message" popups: subscribe to inserts on the conversations
  // already visible in the top-bar preview list, so a reply shows up as a
  // toast + a pulse on the chat icon without needing to reopen the page.
  // Scoped to conversations the user is already a member of (same filter
  // shape as the per-conversation subscription in chats/[id].tsx) rather than
  // an unfiltered table subscription, so it can't surface other clinics'
  // messages even if RLS were ever misconfigured.
  useEffect(() => {
    if (!user?.id || chatRows.length === 0) return;
    const channel = db.channel(`topbar-messages-${user.id}`);
    for (const row of chatRows) {
      channel.on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "clinic_messages", filter: `conversation_id=eq.${row.id}` },
        (payload: any) => {
          const senderId = payload?.new?.sender_id;
          if (!senderId || senderId === user.id) return;
          setChatPulse((n) => n + 1);
          void getConversations({ requesterId: user.id, page: 1, itemsPerPage: 6 })
            .then((res) => setChatRows(res?.conversations ?? []))
            .catch(() => {});
          if (!chatsOpenRef.current) {
            const others = row.members?.filter((m) => m.id !== user.id) ?? [];
            const senderName = others.find((m) => m.id === senderId)?.full_name || (row.kind === "group" ? row.title || "Group chat" : "New message");
            showToast({
              kind: "message",
              title: senderName,
              body: "Sent you a new message",
              onPress: () => router.push(`/chats/${row.id}` as any),
            });
          }
        },
      );
  ...
  }, [user?.id, chatRows.map((row) => row.id).join(",")]);
  ...
  return (
    <>
      <View style={styles.container}>
        <View style={styles.inner}>
          ...
          <View style={styles.rightSection}>
            ...
            <Pressable
              onPress={() => setQuickSearchVisible(true)}
              style={({ hovered, pressed }) => [styles.searchButton, hovered && Platform.OS === "web" ? styles.hover : null, pressed ? styles.pressed : null]}
            >
              <Ionicons name="search-outline" size={18} color={theme.colors.textSecondary} />
              <Text style={styles.searchButtonText}>Search</Text>
              {Platform.OS === "web" ? <Text style={styles.searchButtonKbd}>⌘K</Text> : null}
            </Pressable>
            <Pressable
              onPress={() => {
      ...
      <QuickSearch theme={theme} visible={quickSearchVisible} onClose={() => setQuickSearchVisible(false)} />
    </>
  );
};
...
const createStyles = (theme: any) =>
  StyleSheet.create({
    ...
    searchButtonText: { fontSize: 13, fontWeight: "600", color: theme.colors.textSecondary },
    searchButtonKbd: {
      fontSize: 11,
      fontWeight: "700",
