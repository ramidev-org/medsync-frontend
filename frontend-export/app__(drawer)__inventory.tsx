...
export default function InventoryPage() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      // Matches the specific wording callRpc() throws when a function is
      // absent from the exposed schema (see services/backend.ts), rather
      // than a bare "404" substring - a real HTTP 404 from an unrelated
      // network hiccup shouldn't be mislabeled as "the migration is missing".
      const notFound = msg.toLowerCase().includes('was not found (404)');
      if (notFound) {
        ...
        {migrationMissing && (
          <View style={styles.banner}>
            <Text style={styles.bannerText}>
              Inventory isn&apos;t set up yet: the rpc_get_inventory / rpc_upsert_inventory_item / rpc_adjust_inventory
              functions this page needs aren&apos;t deployed to the database. Ask whoever manages the Supabase project
              to add them (apps/backend/database/) before this page can load or save stock.
            </Text>
          </View>
        )}
