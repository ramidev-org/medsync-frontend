...
export default function ConsultationPage({ standaloneModule = false }: { standaloneModule?: boolean }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // Re-check after the second await: if the user navigated away (or
        // `id`/`user.id` changed) while rpc_open_consultation was in flight,
        // the RPC's server-side effect (opening the session) already
        // happened and can't be undone, but applying its response to local
        // state now would overwrite whatever the newer effect run set.
        if (cancelled) return;
  ...
  const persistConsultation = async (closeAfterSave: boolean) => {
    ...
    // Use the same lenient parser as buildObservationPayload (accepts
    // comma/decimal input) instead of the previous strict /^\d+$/ check,
    // which silently dropped any non-integer entry (e.g. "120.5" or "120,5")
    // here while still keeping it in speciality_payload - the two could
    // disagree with no feedback to the user. systolic_bp/diastolic_bp are
    // integer columns, so round rather than reject.
    const [sysStr, diaStr] = String(vitals.tension || "").split("/").map((x) => x.trim());
    const sysNum = parseOptionalNumber(sysStr);
    const diaNum = parseOptionalNumber(diaStr);
    const systolic = sysNum != null ? Math.round(sysNum) : null;
    const diastolic = diaNum != null ? Math.round(diaNum) : null;
  ...
  if (loading) {
    return (
      <View style={[styles.page, { backgroundColor: consultationTheme.colors.background }]}>
        <AnimatedLoading transparent label="Chargement de la consultation..." />
      </View>
    );
  }
  if (!appointment) {
    return (
      <View style={[styles.page, { backgroundColor: consultationTheme.colors.background, padding: 20, alignItems: "center", justifyContent: "center", gap: 10 }]}>
        <Ionicons name="document-text-outline" size={28} color={consultationTheme.colors.textSecondary} />
        <Text style={{ color: consultationTheme.colors.text, fontWeight: "700", fontSize: 16 }}>Consultation introuvable</Text>
        <TouchableOpacity onPress={() => router.push("/visits")} style={{ marginTop: 4, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 10, backgroundColor: consultationTheme.colors.primary }}>
          <Text style={{ color: consultationTheme.colors.textOnPrimary, fontWeight: "700" }}>Retour aux visites</Text>
        </TouchableOpacity>
      </View>
    );
  }
