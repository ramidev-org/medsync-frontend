...
export default function PatientMedicalDocument() {
  ...
  useEffect(() => {
    if (!patientId) return;
    // Without this guard, navigating quickly between two patients' medical
    // records could let a slow response for the FIRST patient resolve after
    // the SECOND patient's faster response, silently overwriting the screen
    // with the wrong patient's medical data.
    const cancelledRef = { current: false };
    fetchPatientData(cancelledRef);
    return () => {
