...
export default function LabWorkspaceScreen() {
  ...
  // loadData has several independent call sites (the mount effect, the
  // "Actualiser" chip, and the post-save reloads after creating an order or
  // saving results) - a single cancelledRef isn't enough to keep them from
  // racing each other. A monotonic sequence number lets each call detect if
  // a newer call has started before it applies its own result, so the last
  // *started* refresh always wins regardless of which one's network request
