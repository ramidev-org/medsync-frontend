...
export default function Layout() {
  ...
  // On web, a manually-collapsed drawer still temporarily expands while the
  // pointer is over it (VS Code-style activity bar behavior) so labels are
  // reachable without permanently reclaiming the width.
  const [hovering, setHovering] = useState(false);
  const displayExpanded = expanded || (Platform.OS === "web" && hovering);
  const drawerWidth = displayExpanded ? 220 : 88;
  const hoverHandlers =
    Platform.OS === "web"
      ? ({ onMouseEnter: () => setHovering(true), onMouseLeave: () => setHovering(false) } as any)
      : null;
    ...
    <Drawer
      screenOptions={{
        ...
        drawerStyle: {
          ...
          ...(Platform.OS === "web"
            ? ({ boxShadow: "0px 12px 28px rgba(15,23,42,0.06)", transitionProperty: "width", transitionDuration: "160ms", transitionTimingFunction: "ease", overflow: "hidden" } as any)
            : null),
        },
      }}
      drawerContent={(props) => (
        <CustomDrawerContent
          ...
          clinicName={clinic?.name || "MyDoctor"}
...
function CustomDrawerContent({ expanded, pinned, setExpanded, theme, state, role, isClinicAdmin, clinicName, userName, onMouseEnter, onMouseLeave }: any) {
  ...
  return (
    <DrawerContentScrollView
      ...
      contentContainerStyle={{ flex: 1, paddingTop: 16, paddingHorizontal: 10 }}
    >
      ...
      <View style={{ paddingHorizontal: expanded ? 10 : 6, marginBottom: 10 }}>
        <TouchableOpacity
          onPress={() => setExpanded(!pinned)}
          accessibilityLabel={pinned ? "Collapse navigation" : "Pin navigation open"}
          style={{ width: 44, height: 44, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: pinned ? theme.colors.primarySoft : theme.colors.background, borderWidth: 1, borderColor: theme.colors.border }}
        >
          <Ionicons name={pinned ? "chevron-back" : "menu"} size={20} color={pinned ? theme.colors.primary : theme.colors.textSecondary} />
        </TouchableOpacity>
      </View>
