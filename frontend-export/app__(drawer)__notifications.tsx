...
export default function NotificationsPage() {
  ...
  const markAsRead = async (ids: string[]) => {
    ...
    try {
      ...
      // Don't resync via load() here - it resets `error` to null at its own
      // start, which would race with (and immediately erase) the message
      // we're about to show. The optimistic unread=false above stays as-is;
      // the user just also sees that the save itself failed.
      setError(err instanceof Error ? err.message : "Failed to save read status");
    }
  };
      ...
      {error ? (
        <View style={styles.errorBox}>
          ...
          <TouchableOpacity onPress={() => load()}>
            <Text style={{ color: theme.colors.error, fontWeight: "700" }}>Retry</Text>
          </TouchableOpacity>
        </View>
      ) : null}
