    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) {
          setCounts(null);
          setAgenda([]);
          setError("Could not load today's dashboard data. Pull to refresh or try again shortly.");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  if (loading && !counts) {
    return (
      <View style={[styles.page, { backgroundColor: theme.colors.background }]}>
        <TopBar theme={theme} />
        <AnimatedLoading transparent label="Loading your desk..." />
      </View>
    );
  }
      ...
      <ScrollView contentContainerStyle={styles.container}>
        {error ? (
          <View style={localStyles.errorBanner}>
            ...
            <Text style={[localStyles.errorBannerText, { color: theme.colors.error }]}>{error}</Text>
          </View>
        ) : null}
...
const localStyles = StyleSheet.create({
  errorBanner: {
    ...
    borderRadius: 12,
    marginBottom: 14,
    backgroundColor: "rgba(224, 86, 91, 0.10)",
    borderWidth: 1,
    borderColor: "rgba(224, 86, 91, 0.22)",
  },
  errorBannerText: {
    fontSize: 13,
    fontWeight: "600",
