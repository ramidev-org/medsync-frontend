...
export const appTheme = {
  ...
  colors: {
    ...
    // Near-white tints between surface and surfaceVariant - large calm
    // areas (side panels, card headers, dashed placeholder cards) where
    // surfaceVariant reads as too grey against white.
    surfaceSubtle: colors.light.surfaceSubtle,
    surfaceRaised: colors.light.surfaceRaised,
    card: colors.light.card,
    text: colors.light.text,
    textSecondary: colors.light.textSecondary,
    border: colors.light.border,
    // Hairline divider for dense list rows where `border` stacks into
    // visually heavy banding.
    borderSubtle: colors.light.borderSubtle,
    // Structural divider between chrome sections.
    borderMuted: colors.light.borderMuted,
    // Soft blue border/fill for "empty but available" affordances.
    borderAccent: colors.light.borderAccent,
    // Unfilled segment of a seat/quota meter.
    meterTrackEmpty: colors.light.meterTrackEmpty,
