That error happens when the seed statements (which reference the internal PL/pgSQL variable v_admin_id) are being executed outside the seed_large_dev() function.

Do this instead:

Create a real admin user in Supabase Auth (Dashboard → Auth → Users).
Run the whole file database/sql/020_seed_large_dev.sql in the SQL editor (it only creates tables/policies + the seed function).
Then run ONE of these (replace with your real values):
select public.seed_large_dev('admin@yourdomain.com');
or (by user UUID):

select public.seed_large_dev_by_id('YOUR_AUTH_USER_UUID_HERE');
If you paste the exact SQL statement you ran when you got relation "v_admin_id" does not exist, I can confirm where it went wrong.
...
Review
database/sql/020_seed_large_dev.sql

select public.seed_large_dev_by_id('279e4cf5-b24e-446d-8c61-59ec9a2955c5');
4:14 PM


If seed_large_dev_by_id is created and that Auth user exists, that call should work and return SELECT 1 row (void).

If it errors, run these in the Supabase SQL editor and paste the outputs/errors:

-- 1) Confirm the functions exist
select routine_name
from information_schema.routines
where routine_schema = 'public'
  and routine_name in ('seed_large_dev', 'seed_large_dev_by_id');

-- 2) Confirm the auth user exists + has an email
select id, email
from auth.users
where id = '279e4cf5-b24e-446d-8c61-59ec9a2955c5';
Then try the email version (it gives clearer errors):

select public.seed_large_dev('THE_EMAIL_FROM_QUERY_ABOVE');
Most common cause: the SQL file database/sql/020_seed_large_dev.sql didn’t fully run (so the function wasn’t created), or the auth.users row doesn’t exist / has no email.
